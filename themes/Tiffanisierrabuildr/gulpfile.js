...
const SOURCE = {
	...
	morestyles: [
		'assets/styles/scss/additional-styles/**/*.scss',
	
	],	
	
	styles: [
		'assets/styles/scss/main-styles/**/*.scss'
...
const ASSETS = {
	morestyles: 'assets/styles/css/additional-styles',
	styles: 'assets/styles/css/main-styles',
...
gulp.task('morestyles', function() {
	return gulp.src(SOURCE.morestyles)
		.pipe(plugin.plumber(function(error) {
            gutil.log(gutil.colors.red(error.message));
            this.emit('end');
        }))
		...
		.pipe(plugin.cssnano({safe: true, minifyFontValues: {removeQuotes: false}}))
		.pipe(plugin.sourcemaps.write('.'))
		.pipe(gulp.dest(ASSETS.morestyles))
		.pipe(touch())
		.pipe(browserSync.stream());		
});
...
gulp.task('browsersync', function() {
    ...
    gulp.watch(SOURCE.morestyles, gulp.parallel('morestyles')).on('change', browserSync.reload);
...
gulp.task('watch', function() {
	...
	// Watch .css files
	gulp.watch(SOURCE.morestyles, gulp.parallel('morestyles'));
...
gulp.task('default', gulp.parallel('styles', 'morestyles', 'scripts', 'images'));